...
public class EnemyAI : MonoBehaviour
{
    ...
    public virtual void Start()
    {
        ...
        {
            AI.GetComponent<EnemyAI>().FindPlayers();
        }
    ...
    public void FindPlayers()
    {
        ...
        if (!friendlyAI)
        {
            ...
            if (AIplayers.Count != 0)
            {
                ...
                {
                    AI.GetComponent<EnemyAI>().GetClosestPlayer();
                }
    ...
    void ApplyDamage(int damage)
    {
        ...
        if ( actualState != AIState.Dead)
        {
            ...
            if (weapon.Type == Weapon.WT.Melee)
                SetCanAttack(false);
    ...
    void PodDestruction(Vector3 hitDir)
    {
        ...
        if (destroyOnDead)
        {
            DestroyTimer DestroyScript = GetComponent<DestroyTimer>();
    ...
    void InstantiateWeapon()
    {
        if (AssignedWeapon && WeaponGrip)
        {
            ...
            weapon = InstWeapon.GetComponent<Weapon>();
    ...
    public virtual void ApplyState()
    {
        switch (actualState)
        {
            ...
            case AIState.FriendlyFollow:
                if (enemyAnimator.GetBool("Aiming") == true)
                {
                    enemyAnimator.SetBool("Aiming", false);
                }
        ...
        else if (friendlyAI)
        {
            if (Physics.Raycast(actualSensorPos, targetDir, out hit, awarnessDistance, finalMask))
            {
                ...
                else if (hit.collider.CompareTag("Enemy"))
                {
                    ...
                    if (hit.collider.GetComponent<EnemyAI>().dead == true)
                    {
                        playerIsVisible = false;
                        actualState = AIState.FriendlyFollow;
                    }
                ...
                if (playerIsVisible)
                {
                    ...
                    else if (playerActualDistance <= aimingDistance /*&& agent.remainingDistance != 0*/)
                    {
                        if (playerActualDistance <= attackDistance)
                        {
                            if (!friendlyAI)
                            {
                                if (playerTransform && playerTransform.GetComponent<TopDownCharSettings>())
                                {
                                    if (playerTransform.GetComponent<TopDownCharSettings>().isDead == false)
                                ...
                                else if (playerTransform && playerTransform.GetComponent<EnemyAI>())
                                {
                                    if (playerTransform.GetComponent<EnemyAI>().dead == false)
                                        actualState = AIState.Attacking;
                                    else
                                    {
                                        FindPlayers();
                            ...
                            {
                                if (playerTransform && playerTransform.GetComponent<EnemyAI>().dead == false)
                                    actualState = AIState.Attacking;
                                else
                                {
                                    FindPlayers();
